...
1. 支持多系统(windows x64/Linux x64/Linux arm64) 等多种系统(已实现)
2. 对主流VPS环境进行适配(已实现)
3. 能够长期(超过180天)稳定运行(已实现)
4. 支持Javscript/lua等常见语言，并能容易的扩展到其他语言上(已实现)
5. 提供WEB 界面和APP端/桌面应用的连接方式(已实现)
6. 可以提供原生GUI方式进行使用(未实现)
7. 通过Hellswitch服务。可以将多个VPS上的客户端之间建立信息共享机制(已实现)
...
* Core 核心层，子项目中的 核心接口，本质是将 功能的上下文和函数做一个封装，作为一个统一稳定的对外接口暴露出来。因为个人比较反继承，所以会有这么个类。
